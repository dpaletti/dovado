...
def compute_hv_nadir_as_list(data_frame, min_list, max_list):
    #Normalize data
    labels = data_frame.columns.values
    i = 0
    for l in labels:
        data_frame[l] = (data_frame[l]-min_list[i]) / (max_list[i]-min_list[i])
        i = i + 1
    points = data_frame.values.tolist()
    H = len(data_frame.columns.values)-1
    r = 1 + 1/H
    nadir = (H+1) * [r]
...
def main():
    ...
    hv, nadir = compute_hv_nadir_as_list(objfr, min_list, max_list)
