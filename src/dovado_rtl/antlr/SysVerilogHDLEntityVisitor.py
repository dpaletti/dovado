...
class SysVerilogHDLEntityVisitor(SysVerilogHDLVisitor):
    ...
    def visitInout_description(
        ...
        for p in self.visitPort_description(ctx.port_description()):
            self.entities[-1].add_port(Port(p[0], "inout", p[1]))
    ...
    def visitInput_description(
        ...
        for d in chain(
            ...
            self.entities[-1].add_port(Port(d[0], "input", d[1]))
    ...
    def visitOutput_description(
        ...
        if ctx.port_description():
            for p in self.visitPort_description(ctx.port_description()):
                self.entities[-1].add_port(Port(p[0], "output", p[1]))
        ...
        for d in chain(
            ...
            self.entities[-1].add_port(Port(d[0], "output", d[1]))
    ...
    def visitNet_declaration(
        ...
        for v in self.visitList_of_variable_descriptions(
            ...
            p_type = self.visitNet_type(ctx.net_type())
    ...
    def visitNet_type(self, ctx: SysVerilogHDLParser.Net_typeContext) -> str:
        if ctx.Supply0():
            return ctx.Supply0().getText()
        if ctx.Supply1():
            return ctx.Supply1().getText()
        if ctx.Tri():
            return ctx.Tri().getText()
        if ctx.Tri_and():
            return ctx.Tri_and().getText()
        if ctx.Tri_or():
            return ctx.Tri_or().getText()
        if ctx.Tri_reg():
            return ctx.Tri_reg().getText()
        if ctx.Tri0():
            return ctx.Tri0().getText()
        if ctx.Tri1():
            return ctx.Tri1().getText()
        if ctx.Uwire():
            return ctx.Uwire().getText()
        if ctx.Wire():
            return ctx.Wire().getText()
        if ctx.Wand():
            return ctx.Wand().getText()
        if ctx.Wor():
            return ctx.Wor().getText()
        if ctx.NONE():
    ...
    def visitList_of_hierarchical_variable_descriptions(
        ...
        vds = [
            self.visitHierarchical_variable_description(
