...
class XdcFrameHandler(FrameHandler):
    def __init__(
        ...
        FrameHandler.__init__(self, placeholder, frame_path, out_path)
...
class TclFrameHandler(FrameHandler):
    ...
    def setup_incremental(self, is_incremental: IsIncremental):
        ...
        if is_incremental.synthesis:
            path = str(self.config.get_config("TCL_DIR")) + str(
                self.config.get_config("SYNTHESIS")
            )
        ...
        if is_incremental.implementation:
            path = str(self.config.get_config("TCL_DIR")) + str(
                self.config.get_config("IMPLEMENTATION")
            )
            implementation: str = Path(path).read_text()
    ...
    def __vhdl_fill_box(
        ...
        input_mapping = [
            ...
            + (
                ...
                else " => " + str(port.get_type()) + "'((others => '1')),\n"
            )
            for port in self.ports
            if (
                str(port.get_direction()) == "input"
