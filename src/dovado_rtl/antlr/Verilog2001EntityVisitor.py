...
class Verilog2001EntityVisitor(Verilog2001Visitor):
    ...
    def visitSpecparam_declaration(
        ...
        for p in self.visitList_of_specparam_assignments(
            ...
            self.entities[-1].add_parameter(Parameter(p, "specparam"))
            ...
            for p in self.visitList_of_port_identifiers(
                ...
                self.entities[-1].add_port(
                    Port(
                        ...
                        "output",
                        (
                            ("reg" if ctx.getText()[6:9] == "reg" else "wire")
                            if not ctx.net_type()
                            else self.visitNet_type(ctx.net_type())
                        )
    ...
    def visitNet_type(self, ctx: Verilog2001Parser.Net_typeContext) -> str:
        return ctx.getText()
    ...
    def visitParameter_declaration_(
        ...
        if ctx.range_():
            for pa in self.visitList_of_param_assignments(
                ...
                print("Vector parameter " + pa + " is ignored")
        else:
            for pa in self.visitList_of_param_assignments(
                ctx.list_of_param_assignments()
            ):
                if "integer" in ctx.getText()[9 : 9 + len("integer")]:
                    self.entities[-1].add_parameter(Parameter(pa, "integer"))
                if "time" in ctx.getText()[9 : 9 + len("time")]:
                    self.entities[-1].add_parameter(Parameter(pa, "time"))
                else:
                    print(
                        "Parameter "
                        + pa
                        + " without type specification is assumed integer"
                    )
                    self.entities[-1].add_parameter(Parameter(pa, "integer"))
